...
if __name__ == "__main__":
    ...
    l.log( l.INFO, "--------------------------------------------------------------------------------" )
    l.log( l.INFO, "OGC.Engineering - ogc-network-mqtt-client-application.py" )
    l.log( l.INFO, "--------------------------------------------------------------------------------" )

    # TODO: create a unique name for this client for use in client creation
    client = paho.Client( "client-%s" % ( hostname ) )
    ...
    while True:
        ...
        buffer = "{\"msg_type\":\"stat\",\"msg_id\":0,\"data_type\":\"client\",\"device_name\":\"%s\",\"device_group\":\"demo\",\"device_uptime\":\"%s\",\"device_status\":\"OK\"}" % ( hostname, time.monotonic() )
        client.publish( "/network/broadcasts", buffer )
        time.sleep( 5 )  # repeat every minute
